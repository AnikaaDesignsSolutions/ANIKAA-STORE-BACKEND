...
export const POST = async (req: MedusaRequest<UpdateLineItemRequestBody>, res: MedusaResponse) => {
  ...
  try {
    ...
    const { id, design_preference, design_images, material_images, measurement_values } = req.body;

    console.log("id: ", id);
    console.log("design_preference: ", design_preference);
    console.log("design_images: ", design_images);
    console.log("material_images: ", material_images);
    ...
    if (material_images !== undefined) {
      updateData.material_images = material_images;
    }
